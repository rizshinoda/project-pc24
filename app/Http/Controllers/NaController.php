...
class NaController extends Controller
{
    ...
    public function approveupgrade($id)
    {
        ...
        return redirect()->route('na.upgrade_show', $id)
    ...
    public function storeProgressUpgrade(Request $request, $id)
    {
        ...
        if ($request->has('action') && $request->action === 'complete') {
            ...
            foreach ($adminUsers as $admin) {
                ...
                } else if ($admin->is_role == 6) { // Role Admin
                    $url = route('na.upgrade_show', ['id' => $getUpgrade->id]) . '#upgrade'; // Tambahkan #no_spk untuk Admin
                }
    ...
    public function approvedowngrade($id)
    {
        ...
        // Redirect ke halaman sebelumnya dengan pesan sukses
        return redirect()->route('na.downgrade_show', $id)
        ...
        if ($request->has('action') && $request->action === 'complete') {
            ...
            foreach ($adminUsers as $admin) {
                ...
                if ($admin->is_role == 1) { // Role PSB
                    ...
                    $url = route('na.downgrade_show', ['id' => $getDowngrade->id]) . '#downgrade'; // Tambahkan #no_spk untuk Admin
                }
    ...
    public function storeDisable(Request $request, $id)
    {
        ...
        return redirect()->route('na.dismantle_show', $id)->with('success', 'Eth ke arah Client berhasil di Disable');
    }
    ...
    public function storeProgressRelokasi(Request $request, $id)
    {
        ...
        // Redirect ke view upgrade atau detail upgrade berdasarkan aksi
        if ($request->action === 'complete') {
            return redirect()->route('na.relokasi.show', $id)->with('success', 'Progress berhasil diselesaikan.');
        }

        return redirect()->route('na.relokasi.show', $id)->with('success', 'Progress berhasil ditambahkan.');
    }
    ...
    public function storeProgressGantivendor(Request $request, $id)
    {
        ...
        if ($request->action === 'complete') {
            return redirect()->route('na.gantivendor.show', $id)->with('success', 'Progress berhasil diselesaikan.');
        }

        return redirect()->route('na.gantivendor.show', $id)->with('success', 'Progress berhasil ditambahkan.');
    }
    ...
    public function storerequest(Request $request)
    {
        ...
        foreach ($gaUsers as $gaUser) {
            ...
            Notification::create([
                'user_id' => $gaUser->id,
                'message' => 'Request barang baru diajukan oleh NA: ' . Auth::user()->name,
                'url' => $url, // URL dengan hash #request
            ]);
        }
        return redirect()->route('na.request_barang')->with('success', 'Request barang berhasil diajukan.');
    }

    public function markAsReadNa($id)
    {
    ...
    public function updateRequest(Request $request, $id)
    {
        ...
        return redirect()->route('na.request_barang')->with('success', 'Request barang berhasil diperbarui.');
    }
...
//ambilDataRole(): Fungsi ini mengambil data role pengguna dan menyiapkan folder view berdasarkan role.
//renderView(): Fungsi ini menentukan view secara dinamis dengan mencocokkan nama folder berdasarkan role pengguna dan menggabungkannya dengan nama view yang diinginkan.
//Method Controller: Setiap method untuk view (dashboard(), instalasi(), dll.) sekarang memanggil renderView() dengan parameter nama view, membuat kode lebih bersih dan konsisten.
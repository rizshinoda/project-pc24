...
class PsbController extends Controller
{
    ...
    public function printJasaPDF($id)
    {
        $getInstall = WorkOrderInstall::findOrFail($id);
        $tanggalInstalasi = now()->format('d-m-Y');

        $formattedDate = Carbon::parse($tanggalInstalasi)->translatedFormat('l, d F Y');
        $templatePath = resource_path('pdf/jasa.pdf');
    ...
    public function printPocPDF($id)
    {
        $getInstall = WorkOrderInstall::findOrFail($id);
        $tanggalInstalasi = now()->format('d-m-Y');

        $formattedDate = Carbon::parse($tanggalInstalasi)->translatedFormat('l, d F Y');
        $templatePath = resource_path('pdf/poc.pdf');
    ...
    public function approvepoc($id)
    {
        ...
        // Redirect ke halaman sebelumnya dengan pesan sukses
        return redirect()->route('psb.poc_show', $id)
        ...
        if ($request->has('action') && $request->action === 'complete') {
            ...
            foreach ($adminUsers as $admin) {
                ...
                if ($admin->is_role == 1) { // Role PSB
                    ...
                    $url = route('psb', ['id' => $getInstall->id]) . '#POC'; // Tambahkan #no_spk untuk Admin
                }
    ...
    public function approvejasa($id)
    {
        ...
        // Redirect ke halaman sebelumnya dengan pesan sukses
        return redirect()->route('psb.jasa_show', $id)
        ...
        if ($request->has('action') && $request->action === 'complete') {
            ...
            foreach ($adminUsers as $admin) {
                ...
                if ($admin->is_role == 1) { // Role PSB
                    ...
                    $url = route('psb.jasa_show', ['id' => $getInstall->id]) . '#Jasa'; // Tambahkan #no_spk untuk Admin
                }

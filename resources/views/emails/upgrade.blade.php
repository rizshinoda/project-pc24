...
<head>
    <meta charset="UTF-8">
    <title>
        Work Order Upgrade @if($getUpgrade->onlineBilling)
        / Client {{ $getUpgrade->onlineBilling->pelanggan->nama_pelanggan ?? '-' }} {{ $getUpgrade->onlineBilling->nama_site ?? '' }}
        @endif
    </title>
</head>
...
<body style="font-family: Arial, sans-serif; font-size: 14px; color: #333; line-height: 1.4;">
    ...
    <table cellpadding="4" cellspacing="0" border="0"
        ...
        <tbody>
            ...
            @if ($billing = $getUpgrade->onlineBilling)
            <tr>
    ...
    <p style="margin: 6px 0;">
        📎 <a href="{{ url($basePath . $getUpgrade->id) }}">
            Lihat Detail Permintaan
        </a>
    </p>
    <br>
    <p style="margin-top: 10px;">Warm regards,<br>
        {{ $getUpgrade->admin->name ?? 'User Pengaju' }}
    </p>
